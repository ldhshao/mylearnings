...
QString JSONObject::quote(const QString& Text)
{
    ...
    std::basic_string<wchar_t> ws = Text.toStdWString();
#else
    std::basic_string<wchar_t> ws = Text.toStdWString();
#endif
    ...
    if ( *ptr==L'\"' )
    {
        ...
        while(*ptr!=L'\"' && *ptr)
        {
            if ( *ptr == L'\\' )
            {
                ...
                {
                    ptr++;
                    QString s = QString::fromStdWString(std::basic_string<wchar_t>(ptr, 4));
            ...
            if ( *ptr==L'{' )
            {
                ...
                if ( *ptr==L'[' )
                {
                         ...
                         && (*ptr==L'+'||*ptr==L'-'||*ptr==L'.'||(*ptr>=L'0'&&*ptr<=L'9')||*ptr==L'E'||*ptr==L'e') )
                    {
                        ...
                        if ( std::basic_string<wchar_t>(ptr, 4).compare(L"true")==0 )
                        {
                            add(newKey(Key), true);
                            ptr+=4;
                        }else
                            if ( std::basic_string<wchar_t>(ptr, 5).compare(L"false")==0 )
                            {
                                add(newKey(Key), false);
                                ptr+=5;
                            }else
                                if ( std::basic_string<wchar_t>(ptr, 4).compare(L"null")==0 )
                                {
                                    add(newKey(Key));
                                    ptr+=4;
                                }

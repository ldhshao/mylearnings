...
bool QkeyTools::eventFilter(QObject *obj, QEvent *event)
{
    if (event->type() == QEvent::MouseButtonPress) {
        if (obj == ui->labCh0) {
            ...
            setCandidateWords(9);
        } else if (!currentEditType.isEmpty()) {
            if (obj == currentWidget || obj == currentLineEdit ||
                obj == currentTextEdit || obj == currentPlain  ||
                obj == currentBrowser)
                this->show();
            //qDebug()<<currentEditType<<" obj "<<obj<<" closeBtn "<<ui->CloseBtn;
        }
